...
/******************************************************************************\
    ...
    final class dl\FileMutex

	Файловый эмулятор мьютекса.
	Использовать, если недоступны функции межпроцессного взаимодействия
	System V и недоствпно расширение доступа к сегментам разделяемой памяти.
...
final class FileMutex extends Mutex {
	...
	* а создание папки приводит к ошибке или непредсказуемому результату (например, в dl\IO::isdir()).
	*/
